...
const global = () => {
  return {
    ...
    plugins: [
      new CopyPlugin({
        patterns: [
          { from: path.resolve("client/cl_controls.lua"), to: path.resolve(buildPath, '../client/cl_controls.lua'), },
          { from: path.resolve("../config.json"), to: path.resolve(buildPath, '../config.json') },
          { from: path.resolve("../fxmanifest.lua"), to: path.resolve(buildPath, '../fxmanifest.lua') },
        ],
      })
    ]
  };
