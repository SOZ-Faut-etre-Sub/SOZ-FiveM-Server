import {SocietyEvents, PreDBSociety, SocietyMessage, DBSocietyUpdate} from '../../../typings/society';
...
onNetPromise<DBSocietyUpdate, boolean>(SocietyEvents.UPDATE_SOCIETY_MESSAGE, (reqObj, resp) => {
  SocietyService.updateSocietyMessage(reqObj, resp).catch((e) => {
    societiesLogger.error(
      `Error occured in updateSocietyMessage event (${reqObj.source}), Error:  ${e.message}`,
    );
    resp({ status: 'error', errorMsg: 'INTERNAL_ERROR' });
  });
});
