...
export const useSocietyMessagesService = () => {
    ...
    const handleMessageBroadcast = ({id, conversation_id, source_phone, message, position, isTaken, takenBy, takenByUsername, isDone, createdAt, updatedAt}) => {
        if (visibility && pathname.includes('/society-messages')) {
            return;
        }

        setNotification({message});
        updateLocalMessages({id, conversation_id, source_phone, message, position, isTaken, takenBy, takenByUsername, isDone, createdAt, updatedAt});
    };
