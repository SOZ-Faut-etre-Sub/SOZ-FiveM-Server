...
RegisterNetEvent('police:confiscateMoney', function(target)
    ...
    if Target.Functions.RemoveMoney("marked_money", markedAmount) then
        ...
        TriggerClientEvent('hud:client:DrawNotification', Player.PlayerData.source, string.format("Vous avez confisqué ~g~%s$~s~ d'argent sale à ~y~%s~s~", markedAmount, Target.Functions.GetName()))
        TriggerClientEvent('hud:client:DrawNotification', Target.PlayerData.source, string.format("~r~%s$~s~ d'argent sale ont été confisqués par ~y~%s~s~", markedAmount, Player.Functions.GetName()))
    else
