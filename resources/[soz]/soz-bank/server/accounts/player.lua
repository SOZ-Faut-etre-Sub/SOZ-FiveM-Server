...
function PlayerAccount:load(id, owner)
    local created = false
    local result = MySQL.Sync.fetchScalar("SELECT money FROM bank_accounts WHERE account_type = 'player' AND accountid = ? AND citizenid = ?", {id, owner})
    if result == nil then
        MySQL.insert.await("INSERT INTO bank_accounts (accountid, citizenid, account_type, money) VALUES (?, ?, 'player', ?)", {id, owner, 0})
        created = true
    end
