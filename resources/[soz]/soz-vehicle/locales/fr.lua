local Translations = {
    error = {
        no_vehicles = "~r~Vous n'avez aucun véhicule dans ce garage!",
        not_in_parking = "~r~Pas de véhicule dans le parking ou vous n'êtes jamais monté dedans",
        not_impound = "~r~Votre véhicule n'est pas à la fourrière",
        not_owned = "~r~Le véhicule ne vous appartient pas",
        not_correct_type = "~r~Vous ne pouvez pas stocker ce type de véhicule ici.",
        not_enough = "~r~Pas assez d'argent",
        no_garage = "~r~Aucun",
        no_vehicles_impounded = "~r~Vous n'avez aucun véhicule en fourrière!",
        vehicle_at_depot = "~r~Votre véhicule doit être à la fourrière!",
        impounded_by_police = "~r~Ce véhicule a été mis en fourrière par la police!",
        someone_inside = "~r~Le véhicule n'a pu être rangé, quelqu'un se trouve sans doute dedans.",
    },
    success = {vehicle_parked = "Véhicule garé !", vehicle_out = "Véhicule sorti !"},
    menu = {
        header = {
            house_car = "Garage de propriété %{value}",
            public_car = "Garage Public %{value}",
            public_sea = "Hangar à bateaux Public %{value}",
            public_air = "Hangar Public %{value}",
            job_car = "Garage de fonction %{value}",
            job_sea = "Hangar à bateaux de fonction %{value}",
            job_air = "Hangar de fonction %{value}",
            gang_car = "Garage de Gang %{value}",
            gang_sea = "Hangar à bateaux de Gang %{value}",
            gang_air = "Hangar de Gang %{value}",
            depot_car = "Fourrière %{value}",
            depot_sea = "Fourrière %{value}",
            depot_air = "Fourrière %{value}",
            vehicles = "Véhicules disponibles",
            depot = "%{value} [ %{value2} ] - Coût: $%{value3}",
            public = "%{value} [ %{value2} ]",
            private = "%{value} [ %{value2} ] - Coût: $%{value3}",
            entreprise = "%{value} [ %{value2} ]",
        },
        leave = {
            car = "⬅ Quitter le garage",
            sea = "⬅ Quitter le hangar à bateaux",
            air = "⬅ Quitter le Hangar",
            depot = "⬅ Quitter la fourrière",
        },
        text = {
            vehicles = "Voir les véhicules stockés",
            depot = "État: %{value}<br>Réservoir: %{value2} | Moteur: %{value3} | Carrosserie: %{value4}",
            garage = "État: %{value}<br>Réservoir: %{value2} | Moteur: %{value3} | Carrosserie: %{value4}",
        },
    },
    status = {out = "Dehors", garaged = "Garé dans un parking", impound = "En Fourrière"},
    info = {
        car_e = "~g~E~w~ - Garage",
        sea_e = "~g~E~w~ - Hangarà bateaux",
        air_e = "~g~E~w~ - Hangar",
        park_e = "~g~E~w~ - Ranger le véhicule",
        house_garage = "Garage Personnel",
    },
}

Lang = Locale:new({phrases = Translations, warnOnMissing = true})
