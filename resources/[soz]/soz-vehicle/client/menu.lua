...
local function EngineMenu(vehicle)
    local engine = vehicleMenu:AddSlider({ label = 'Contact du moteur', value = nil, values = {
        { label = 'Allumé', value = true },
...
local function SpeedLimiterMenu(vehicle)
    local speed = vehicleMenu:AddSlider({ label = 'Limitateur de vitesse', value = nil, values = {
...
local function DoorManagementMenu(vehicle)
    ...
    for i = 0, 5 do
        if not IsVehicleDoorDamaged(vehicle, i) and GetIsDoorValid(vehicle, i) then
            local door = doorMenu:AddCheckbox({ label = doorName[i + 1], value = GetVehicleDoorAngleRatio(vehicle, i) >= 0.5 })
