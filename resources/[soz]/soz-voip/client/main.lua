...
local function RefreshState(state)
    ...
    for _, config in pairs(state.players) do
        ...
        --print("SET PLAYER", voiceTarget, config.serverId, config.volume)
    end
...
local function ApplyFilters(players)
    for _, player in pairs(players) do
        ...
        if filterType == nil then
            FilterRegistryInstance:disconnect(player.serverId)
        else
            FilterRegistryInstance:update(player.serverId, filterType, player)
        end
    end
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        state.players = updateSpeakers(state.players, SecondaryLongRadioModuleInstance:getSpeakers(), "radio_lr_secondary",
