...
AddEventHandler("mumbleConnected", function(address, isReconnecting)
    logger.info("Connected to mumble server with address of %s, is this a reconnect %s", GetConvarInt("voice_hideEndpoints", 1) == 1 and "HIDDEN" or address,
                isReconnecting)

    logger.log("Connecting to mumble, setting targets.")
    -- don't try to set channel instantly, we're still getting data.
    local voiceModeData = Cfg.voiceModes[mode]
    LocalPlayer.state:set("proximity", {index = mode, distance = voiceModeData[1], mode = voiceModeData[2]}, true)
