...
export class VehicleGarageProvider {
    ...
    public async init(): Promise<void> {
        const queries = `
            ...
            UPDATE vehicles v SET v.stock = 0 - (SELECT COUNT(1) as taken FROM player_vehicles WHERE player_vehicles.vehicle = v.model AND player_vehicles.state != 5)  WHERE v.category = 'Electric';
