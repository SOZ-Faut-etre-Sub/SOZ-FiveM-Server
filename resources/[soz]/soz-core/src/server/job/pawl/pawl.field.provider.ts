...
import { getTreeIdentifier, PAWL_FIELD_LIST, PAWL_FIELD_REFILL_DELAY, PawlField } from '../../../shared/field';
...
export class PawlFieldProvider {
    ...
    async loadPawlField() {
        for (const identifier of PAWL_FIELD_LIST) {
            const field = (await this.fieldRepository.createField({
            ...
            for (const tree of field.field) {
                tree.identifier = getTreeIdentifier(field, tree);
            }
        }
    }
}
