...
export const BankCard: FunctionComponent<HealthCardProps> = ({ name, account }) => {
    return (
        ...
        >
            <div className="flex pt-[12vh] pl-[3.3vh] text-2xl uppercase text-[#4fd954] font-kreditblack">
                {account?.replace(/([A-Z\d]{4})/g, '$1 ') ?? ' '}
            </div>
            <p className="pl-[3.3vh] pt-[3.5vh] uppercase italic font-bold text-white font-kreditback">{name}</p>
        </div>
    );
};
