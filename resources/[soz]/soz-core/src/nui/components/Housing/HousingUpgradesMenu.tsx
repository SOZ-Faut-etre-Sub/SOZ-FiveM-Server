...
import { MainMenu, Menu, MenuContent, MenuItemButton, MenuItemSelect, MenuItemSelectOptionBox, MenuTitle } from "../Styleguide/Menu";
...
export const HousingUpgradesMenu: FunctionComponent<HousingUpgradesMenuProps> = ({ data }) => {
  ...
  const onConfirm = () => {
    fetchNui(NuiEvent.HousingUpgradeApartment, {tier, price: tierPrice, zkeaPrice, enableParking: data.enableParking, hasParking: parking, parkingPrice});
  }
        ...
        <MenuContent>
          ...
          >
            {Object.entries(HousingTiers).map(([tier, _]) => {
              ...
              return <MenuItemSelectOptionBox key={value} value={value}>{label}</MenuItemSelectOptionBox>
            })}
          </MenuItemSelect>
          <MenuItemSelect
            title={
            ...
            alignRight
          >
            {data.enableParking && <MenuItemSelectOptionBox value={false}>Désactivé</MenuItemSelectOptionBox>}
