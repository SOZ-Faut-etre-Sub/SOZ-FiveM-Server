...
export const ItemDescription: FunctionComponent<ItemDescriptionProps> = ({ inventoryItem }: ItemDescriptionProps) => {
    ...
    return (
        <div className="p-2 mt-2 text-lsm rounded text-gray-100 bg-black/60 w-full">
            <div className="flex justify-between align-items-center w-full">
                {inventoryItem.metadata?.label && (
                    <strong className="truncate flex-1">
                        {inventoryItem.metadata?.label} <span className="text-2xs">{itemLabel}</span>
                    </strong>
                )}
                {!inventoryItem.metadata?.label && <strong>{itemLabel}</strong>}
                <div className="flex-0">
                    ...
                    {inventoryItem.name === 'crate' && inventoryItem.metadata?.crateElements?.length && (
                        <span>
                            [
                            {getItemWeight(inventoryItem.name, inventoryItem.amount, resolver, inventoryItem.metadata) /
                                1000}
                            /12Kg]
                        </span>
                    )}
                    ...
                    {expiration && (
                        <>
                            ...
                            {!isInventoryItemExpired(inventoryItem) && (
                                <span>[Expire le {expiration.toLocaleDateString('fr-FR', FORMAT_LOCALIZED)}]</span>
                            )}
                        </>
                    )}
                    ...
                    {(inventoryItem.metadata?.crateElements || []).map((element, index) => (
                        <div key={index}>
                            ...
                            {element.metadata?.expiration
                                ? new Date(element.metadata?.expiration).toLocaleDateString('fr-FR', FORMAT_LOCALIZED)
                                : ''}
                            ]
                        </div>
                    ))}
                ...
                currentTime <= expiration && (
                    <div>
                        {!inventoryItem?.metadata?.evidenceInfos?.isAnalyzed && (
                            <>
                                <div>
                                    <strong>Type : </strong> {getTypeLabel(inventoryItem.metadata?.evidenceInfos?.type)}
                                </div>
                                <div>
                        ...
                        {inventoryItem?.metadata?.evidenceInfos?.isAnalyzed && (
                            <>
                                ...
                                <div>
                                    <strong>Informations : </strong> {inventoryItem.metadata.evidenceInfos.generalInfo}
                                </div>
                            </>
                        )}
