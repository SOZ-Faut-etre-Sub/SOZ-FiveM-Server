            ...
            if (lastUse) {
                ...
                if (remainingTime > 0) {
                    if (amountAvailable == 0) {
                        this.notifier.notify(
                            `Limite de retrait atteinte : max. ~b~$${atmAccount.config.maxWithdrawal}~s~ par tranche de ${atmAccount.config.limit / 60000} minutes. Revenez dans ~b~${remainingTime / 60000} minutes~s~.`,
                            'error'
                        );
                        return;
