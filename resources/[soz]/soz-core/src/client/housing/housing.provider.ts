...
export class HousingProvider {
    ...
    @OnNuiEvent<{ tier: number; price: number; zkeaPrice: number, enableParking: boolean, hasParking: number, parkingPrice: number }>(NuiEvent.HousingUpgradeApartment)
    public async upgradeApartment({tier, price, zkeaPrice, enableParking, hasParking, parkingPrice}) {
      ...
      const { apartment: { tier: currentTier }, money: { money } } = player;
      if (tier < currentTier) {
      ...
      if (price > 0 && zkeaPrice > 0) TriggerServerEvent("housing:server:UpgradePlayerApartmentTier", tier, price, zkeaPrice);
      if (enableParking) TriggerServerEvent("housing:server:SetPlayerApartmentParkingPlace", hasParking, parkingPrice);
