...
local function MoneyMenu()
    ...
    inventoryMenu:AddButton({label = "Votre argent", rightLabel = PlayerData.money["cash"] .. "$", value = moneyMenu})
...
local function ItemsMenu(items)
    ...
    for _, item in pairs(items) do
            ...
            "select", function(i)
                ...
                if player ~= -1 and distance < 2.0 then
                    ...
                    if amount and tonumber(amount) > 0 then
                        SetCurrentPedWeapon(PlayerPedId(), "WEAPON_UNARMED", true)
                        TriggerServerEvent(
                            "inventory:server:GiveItem", GetPlayerServerId(player), i.Value, tonumber(amount)
                        )
        ...
        inventoryMenu:AddButton(
            {label = item.label, rightLabel = item.amount, value = itemMenu, description = item.description}
        )
    end
...
RegisterCommand(
    "inventory", function()
        QBCore.Functions.TriggerCallback(
            "inventory:server:openPlayerInventory", function(inventory)
                if inventory ~= nil then
                    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and
                        ...
                        inventoryMenu:SetSubtitle(
                            string.format("%s/%s Kg", inventory.weight / 1000, inventory.maxWeight / 1000)
                        )
...
RegisterNetEvent(
    "inventory:client:UseWeapon", function(weaponData, shootbool)
        ...
        if currentWeapon == weaponName then
            ...
            currentWeapon = nil
        elseif weaponName == "weapon_stickybomb" or weaponName == "weapon_pipebomb" or weaponName ==
            "weapon_smokegrenade" or weaponName == "weapon_flare" or weaponName == "weapon_proxmine" or weaponName ==
            "weapon_ball" or weaponName == "weapon_molotov" or weaponName == "weapon_grenade" or weaponName ==
            "weapon_bzgas" then
        ...
        elseif weaponName == "weapon_snowball" then
            ...
            TriggerEvent("weapons:client:SetCurrentWeapon", weaponData, shootbool)
            QBCore.Functions.TriggerCallback(
                "weapon:server:GetWeaponAmmo", function(result)
