...
RegisterNetEvent(
    "QBCore:Player:SetPlayerData", function(data)
        PlayerData = data
    end
)
...
RegisterNetEvent(
    "inventory:client:openInventory", function(playerInventory, targetInventory)
...
RegisterNUICallback(
    "transfertItem", function(data, cb)
        ...
        if amount and tonumber(amount) then
            QBCore.Functions.TriggerCallback(
                "inventory:server:TransfertItem", function(success, reason, invSource, invTarget)
        ...
        while true do
            for id, storage in pairs(Config.Storages) do
                if storage.owner == nil or (PlayerData.job ~= nil and PlayerData.job.name == storage.owner) then
                    ...
                    if dist <= 80.0 then
                        DrawMarker(
                            27, storage.position.x, storage.position.y, storage.position.z, 0.0, 0.0, 0.0, 0.0, 180.0,
                            0.0, 1.5, 1.5, 1.5, 255, 128, 0, 50, false, false, 2
                        )
...
RegisterNetEvent(
    "inventory:client:updateStorageState", function(name, state)
