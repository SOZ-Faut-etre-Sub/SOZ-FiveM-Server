QBCore.Commands.Add("clearinv", "Clear Players Inventory (Admin Only)", {{name = "id", help = "Player ID"}}, false,
        ...
        if amount > 0 then
            if itemData ~= nil then
                ...
                elseif itemData["type"] == "weapon" then
                    amount = 1
                    metadata.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) ..
                                                  QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) ..
                                                  QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
                end

                Inventory.AddItem(Player.PlayerData.source, itemData["name"], amount, metadata, false,
                                  function(success, reason)
                    if success then
                        TriggerClientEvent("hud:client:DrawNotification", source, string.format(
                                               "Vous avez donné ~o~%s ~b~%s ~s~à ~o~%s", amount, itemData["name"],
                                               GetPlayerName(tonumber(args[1]))))
                    else
                        TriggerClientEvent("hud:client:DrawNotification", source,
                                           "~r~L'objet ne peut pas être donné: " .. reason)
                    end
                end)
            else
