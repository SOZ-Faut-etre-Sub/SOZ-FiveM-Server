QBCore.Functions.CreateCallback(
    "inventory:server:openPlayerInventory", function(source, cb, type, id)
        ...
        if not ply.state.inv_busy then
            ...
            TriggerClientEvent(
                "hud:client:DrawNotification", Player.PlayerData.source, "Inventaire en cours d'utilisation"
            )
            cb(nil)
        end
    end
...
RegisterNetEvent(
    "inventory:server:UseItemSlot", function(slot)
        local Player = QBCore.Functions.GetPlayer(source)
...
RegisterServerEvent(
    "inventory:server:GiveItem", function(target, item, amount)
        local Player = QBCore.Functions.GetPlayer(source)
        ...
        if dist > 2 then
            return TriggerClientEvent(
                       "hud:client:DrawNotification", Player.PlayerData.source, "Personne n'est à portée de vous"
                   )
        end
        ...
        if amount <= item.amount then
            ...
            Inventory.TransfertItem(
                Player.PlayerData.source, Target.PlayerData.source, item.name, amount, item.metadata, item.slot,
                function(success, reason)
                    if success then
                        TriggerClientEvent(
                            "hud:client:DrawNotification", Player.PlayerData.source,
                            string.format("Vous avez donné ~o~%s ~b~%s", amount, item.label)
                        )
                        TriggerClientEvent(
                            "hud:client:DrawNotification", Target.PlayerData.source,
...
RegisterServerEvent(
    "inventory:server:GiveMoney", function(target, amount)
        local Player = QBCore.Functions.GetPlayer(source)
        local Target = QBCore.Functions.GetPlayer(tonumber(target))
        local dist = #(GetEntityCoords(GetPlayerPed(Player.PlayerData.source)) -
                         GetEntityCoords(GetPlayerPed(Target.PlayerData.source)))
        ...
        if Player.Functions.RemoveMoney("cash", amount) then
            ...
            TriggerClientEvent(
                "hud:client:DrawNotification", Target.PlayerData.source, string.format("Vous avez reçu ~g~%s$", amount)
            )
...
QBCore.Functions.CreateUseableItem(
    "driver_license", function(source, item)
