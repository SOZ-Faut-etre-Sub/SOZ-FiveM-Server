                ...
                elseif stage == 3 then
                    ...
                    if (IsControlPressed(0, 32) and not movingForward) and Config.EnableProne then
                        ...
                        TaskPlayAnimAdvanced(
                            ped, "move_crawl", "onfront_fwd", pronepos.x, pronepos.y, pronepos.z + 0.1, 0.0, 0.0,
                            GetEntityHeading(ped), 100.0, 0.4, 1.0, 7, 2.0, 1, 1
                        )
                        Wait(500)
                    elseif (not IsControlPressed(0, 32) and movingForward) then
                        ...
                        TaskPlayAnimAdvanced(
                            ped, "move_crawl", "onfront_fwd", pronepos.x, pronepos.y, pronepos.z + 0.1, 0.0, 0.0,
                            GetEntityHeading(ped), 100.0, 0.4, 1.0, 6, 2.0, 1, 1
                        )
                        Wait(500)
                        movingForward = false
                    end
...
RegisterNetEvent(
    "crouchprone:client:SetWalkSet", function(clipset)
        walkSet = clipset
    end
)
