...
local function RayCastGamePlayCamera(distance)
    ...
    local a, b, c, d, e = GetShapeTestResult(
                              StartShapeTestRay(
                                  cameraCoord.x, cameraCoord.y, cameraCoord.z, destination.x, destination.y,
                                  destination.z, -1, PlayerPedId(), 0
                              )
                          )
    return b, c, e
end
...
AddEventHandler(
    "soz:client:sit", function(data)
        ...
        SetPedCoordsKeepVehicle(
            player, (coords.x - (0.5 * math.sin(angle))), (coords.y + (0.5 * math.cos(angle))), coords.z - 0.6
        )
