    ...
    Citizen.CreateThread(
        function()
            while showCoords do
                ...
                Draw2DText(
                    string.format(
                        "~w~Ped Coordinates:~b~ vector4(~w~%s~b~, ~w~%s~b~, ~w~%s~b~, ~w~%s~b~)", c.x, c.y, c.z, heading
                    ), 4, {66, 182, 245}, 0.4, x + 0.0, y + 0.0
                )
            end
        end
    )
...
local function AdminPanel(menu)
    local noclip = menu:AddCheckbox(
                       {label = "Noclip", value = noclip_check, description = "Active/Désactive le noclip"}
                   )
    local coords = menu:AddCheckbox(
                       {label = "Voir les coords", value = coords_check, description = "Affiche les coords"}
                   )
    ...
    playerlist:On(
        "select", function(item)
            ...
            QBCore.Functions.TriggerCallback(
                "admin:server:getplayers", function(players)
...
local function GenerateMenu()
    if AdminMenu.IsOpen then
        ...
        QBCore.Functions.TriggerCallback(
            "admin:server:isAllowed", function(isAllowed)
                if isAllowed then
