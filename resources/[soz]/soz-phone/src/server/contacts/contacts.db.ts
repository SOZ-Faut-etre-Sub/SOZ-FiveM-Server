...
export class _ContactsDB {
    ...
    async addContact(identifier: string, { display, number }: PreDBContact): Promise<Contact> {
        const query = 'INSERT INTO phone_contacts (identifier, number, display) VALUES (?, ?, ?)';
    ...
    async updateContact(contact: Contact, identifier: string): Promise<any> {
        const query = 'UPDATE phone_contacts SET number = ?, display = ? WHERE id = ? AND identifier = ?';
        await DbInterface._rawExec(query, [contact.number, contact.display, contact.id, identifier]);
    }
