    ...
    return (
        <div>
            {SpawnList.filter(s => s.identifier !== 'default').map(s => (
                <div className={style.icon} style={s.waypoint && {top: s.waypoint.top, left: s.waypoint.left}} onClick={() => updateSpawn(s)}>
...
const LocationPreview: FunctionComponent<ComponentProps<any>> = (props) => {
    ...
    return (
        <section className={style.container} style={props.style}>
            <div className={style.preview} style={{backgroundImage: `url(${spawn.image})`}} />
            <h3 className={style.title}>{spawn.name}</h3>
            <h4 className={style.description}>{spawn.description}</h4>
