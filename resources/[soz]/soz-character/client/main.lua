...
RegisterNetEvent(
    "soz-character:client:closeNUIdefault", function()
        local src = source
...
RegisterNetEvent(
    "soz-character:client:chooseChar", function()
...
RegisterNetEvent(
    "soz-character:client:selectCharacter", function(data)
        DoScreenFadeOut(10)
...
RegisterNetEvent(
    "soz-character:client:NcDataPed", function(source)
        SetEntityAsMissionEntity(charPed, true, true)
        DeleteEntity(charPed)
        QBCore.Functions.TriggerCallback(
            "soz-character:server:GetUserCharacters", function(result)
                if result[1] ~= nil then
                    ...
                    QBCore.Functions.TriggerCallback(
                        "soz-character:server:getSkin", function(model, data)
                            ...
                            if model ~= nil then
                                CreateThread(
                                    function()
                                        ...
                                        charPed = CreatePed(
                                                      2, model, Config.PedCoords.x, Config.PedCoords.y,
                                                      Config.PedCoords.z - 0.98, Config.PedCoords.w, false, true
                                                  )
                                        ...
                                        QBCore.Functions.GetPlayerData(
                                            function(PlayerData)
                                                SetEntityCoords(
                                                    PlayerPedId(), PlayerData.position.x, PlayerData.position.y,
                    ...
                    QBCore.Functions.TriggerCallback(
                        "soz-character:server:GetUserTempCharacters", function(result2)
...
RegisterNetEvent(
    "soz-character:client:createNewCharacter", function(cData2)
        local cData = cData2
        DoScreenFadeOut(150)
